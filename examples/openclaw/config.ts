import type { OpenClawRuntimeConfig } from "./openclaw/types.js";
...
const library = {
  apiHost: envString("OPEN_XIAOAI_LIBRARY_API_HOST", "127.0.0.1"),
  apiPort: envNumber("OPEN_XIAOAI_LIBRARY_API_PORT", 4402),
  indexFile: envString("OPEN_XIAOAI_LIBRARY_INDEX_FILE", ".openclaw-media-library.json"),
  downloadSubdir: envString("OPEN_XIAOAI_LIBRARY_DOWNLOAD_SUBDIR", "music/library"),
  proxy: envOptionalString("OPEN_XIAOAI_LIBRARY_PROXY") ?? "http://127.0.0.1:7890",
  cookiesPath: envOptionalString("OPEN_XIAOAI_LIBRARY_COOKIES_PATH"),
  ytDlpBin: envString("OPEN_XIAOAI_LIBRARY_YTDLP_BIN", "yt-dlp"),
  ffmpegBin: envString("OPEN_XIAOAI_LIBRARY_FFMPEG_BIN", "ffmpeg"),
  ffprobeBin: envString("OPEN_XIAOAI_LIBRARY_FFPROBE_BIN", "ffprobe"),
};

export const kOpenXiaoAIConfig: OpenClawRuntimeConfig = {
  gateway: {
    server: {
    ...
    openclaw: {
      ...
      sessionUser: envString("OPENCLAW_SESSION_USER", "xiaoai-gateway-family"),
      ...
      systemPrompt: [
        "你是家里小爱音箱里的本地家庭语音助手网关会话。",
        "你的最终文本会被网关直接播报，请默认用简短、自然、适合口语的中文回答。",
